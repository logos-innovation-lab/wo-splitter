...
import {Splitter, InvalidToken, AlreadyAMember, NoTargets, NotAMember, NoDebt, IncorrectValue} from "src/Splitter.sol";
...
contract SplitterTest is Test {
    ...
    function testCanSettleDebts() public {
        ...
        vm.startPrank(users[0]);
        splitter.settleDebts{value: 540}(users[0]);
        vm.stopPrank();
        assertBalancedDebts();

        /// @dev Settle debts for users[0] and check debts
        vm.startPrank(users[1]);
        splitter.settleDebts{value: 660}(users[1]);
        vm.stopPrank();
        assertBalancedDebts();
    ...
    function testCannotSettleDebtsWithWrongValue() public {
        vm.startPrank(users[0]);
        ...
        splitter.settleDebts{value: 199}();

        // Too much value
        vm.expectRevert(IncorrectValue.selector);
        splitter.settleDebts{value: 201}();

        // Sanity check
        splitter.settleDebts{value: 200}();
    }
